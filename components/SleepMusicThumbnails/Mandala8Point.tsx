...
export const Mandala8Point: React.FC<GeometryProps> = ({ color1, color2, size = 120 }) => {
  const centerX = size / 2;
  const centerY = size / 2;
  const radius = size * 0.3;

  // 8-point mandala: 8 petals/points around center
  ...
  points.forEach((point, i) => {
    ...
    const midAngle = (i * Math.PI * 2) / 8 + Math.PI / 8;
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className={size >= 300 ? "" : "rounded-2xl"}>
      <defs>
        <linearGradient id={`grad-mandala8-${color1}-${color2}`} x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor={color1} />
          <stop offset="100%" stopColor={color2} />
        </linearGradient>
        <filter id={`glow-mandala8-gold`}>
      ...
      <g filter={`url(#glow-mandala8-gold)`} opacity="0.9">
        {/* Outer circle */}
        ...
        {/* Center circle */}
        <circle cx={centerX} cy={centerY} r={radius * 0.15} fill="#D4A574" opacity="0.9"/>
      </g>
    </svg>
  );
};
